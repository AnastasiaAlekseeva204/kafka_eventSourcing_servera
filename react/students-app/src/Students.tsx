...
export default function Students() {
    ...
    const LIMIT = 30
    const { data, fetchNextPage, hasNextPage, isFetchingNextPage, error: queryError } = useInfiniteQuery<StudentType[]>({
        initialPageParam: 0, 
        getNextPageParam: (lastPage, allPages) => {
            return (lastPage.length === LIMIT) ? allPages.length : undefined
        }, 
        queryKey: ['paginatedStudents'], 
        queryFn: ({ pageParam = 0 }) => getStudentWithPagination({ pageParam: pageParam * LIMIT, limit: LIMIT }),
        staleTime: 30000
    })
